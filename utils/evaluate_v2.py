...
def query_runs(filter_string: str, order_by: str = "params.dataset ASC"):
    client = MlflowClient()
    runs = client.search_runs("870502346412752581", filter_string=filter_string, order_by=[order_by])
    return runs


query = "params.model = 'CNN' and params.layers = '10' and metrics.`prediction accuracy` >= 0.945"

def query_augmented_result(augment_name, augment_params, concat_original,model_name):
    def construct_filter_string(augment_name, augment_params, concat_original):
        params_keys = list(augment_params.keys())
        filter_string = (
            f"params.concat_original = '{concat_original}' and "
    ...
    filter_string = construct_filter_string(augment_name, augment_params, concat_original)
...
def compare_result(
    ...
    datasets = read_UCR_dataset_name(mySelection=dataset_selection, bakeoff=True)
    ...
    for i in query_list:
        ...
        query_result = query_augmented_result(
            i["augment_name"], i["augment_params"], i["concat_original"], model_name
        )
